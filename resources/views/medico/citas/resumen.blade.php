    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-7xl">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-8">
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <a href="{{ route('medico.citas.reservar') }}"
        ...
        <section class="mb-12">
            ...
            <div id="citasConfirmadas" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($citasConfirmadas as $cita)
                    @php
                        $fechaFinCita = \Carbon\Carbon::parse($cita->fecha)->setTimeFrom(
                            \Carbon\Carbon::parse($cita->hora_fin),
                        );
                        $diasRestantes = now()->diffInDays(\Carbon\Carbon::parse($cita->fecha), false);
                    @endphp
                    <div
                        class="cita bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 border-l-4 border-green-500">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="text-xl font-semibold text-gray-700">{{ $cita->paciente->user->name }}</h3>
                            <span
                        ...
                        <div class="mt-3 flex justify-between items-center">
                            <a href="{{ route('medico.citas.detalles', $cita->id) }}"
                                class="mt-2 inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-200">
                            ...
                            @if ($cita->estado == 'confirmada' && now()->greaterThan($fechaFinCita) && now()->diffInHours($fechaFinCita) < 24)
                                <span
                                    class="inline-flex items-center px-2 ms-3 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800 mb-2">
                                    <svg class="h-4 w-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                            ...
                            @if ($diasRestantes <= 0)
                                <span
                                    class="inline-flex items-center px-2 ms-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 mb-3">
                                    <svg class="h-4 w-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                            ...
                            @elseif($diasRestantes <= 2)
                                <span
                                    class="inline-flex items-center px-2 ms-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 mb-3">
                                    <svg class="h-4 w-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                            d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                                            clip-rule="evenodd" />
                                    </svg>
                                    Próxima (en {{ $diasRestantes }} días)
                                </span>
                            @endif
                        </div>
                    </div>
            ...
            <div id="citasPendientes" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($citasPendientes as $cita)
                    @php
                        ...
                        $estaVencida = now()->greaterThan($fechaFinCita);
        
                        // Nueva lógica para días de confirmación
                        $fechaLimiteConfirmacion = $fechaCita->copy()->subDay();
                        $diasParaConfirmar = now()->diffInDays($fechaLimiteConfirmacion, false);
                        $urgenteConfirmar = $diasParaConfirmar <= 2; // Mostrar como urgente cuando faltan 2 días o menos
                    @endphp
        
                    <div class="cita bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 border-l-4 @if ($estaVencida) border-red-500 @else border-yellow-500 @endif">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="text-xl font-semibold text-gray-700">{{ $cita->paciente->user->name }}</h3>
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium @if ($estaVencida) bg-red-100 text-red-800 @else bg-yellow-100 text-yellow-800 @endif">
                                <svg class="h-4 w-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    @if ($estaVencida)
                        ...
                        <div class="mt-3 flex justify-between items-center">
                            @if (!$estaVencida)
                                <a href="{{ route('medico.citas.detalles', $cita->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-200">
                                    <svg class="h-5 w-5 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                            ...
                            @if (!$estaVencida && $diasParaConfirmar >= 0)
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                                    @if ($diasParaConfirmar == 0) bg-red-100 text-red-800 
                                    @elseif($diasParaConfirmar == 1) bg-orange-100 text-orange-800 
                                    @else bg-yellow-100 text-yellow-800 @endif mb-2">
                                    <svg class="h-4 w-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
                                    </svg>
                                    @if ($diasParaConfirmar == 0)
                                        ¡Confirma hoy!
                                    @elseif($diasParaConfirmar == 1)
                                        Confirma hoy o mañana
                                    @else
                                        Faltan ({{ $diasParaConfirmar }} días)
                                    @endif
                                </span>
                            @endif
                        </div>
        ...
        <section class="mb-12">
            ...
            <div id="citasPasadas" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($citasPasadas as $cita)
                    <div
                        class="cita bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 border-l-4 @if ($cita->estado == 'completada') border-blue-500 @elseif($cita->estado == 'cancelada') border-red-500 @else border-gray-500 @endif">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="text-xl font-semibold text-gray-700">{{ $cita->paciente->user->name }}</h3>
                            <span
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filtrarCitas() {
                ...
                secciones.forEach(seccion => {
                    ...
                    citas.forEach(cita => {
                        const pacienteElement = cita.querySelector('h3');
                        ...
                        const paciente = pacienteElement ? pacienteElement.textContent
                            .toLowerCase() : '';
                        ...
                        if (criterio && texto) {
                            ...
                            if (criterio === 'paciente' && paciente.includes(texto)) {
                                coincide = true;
